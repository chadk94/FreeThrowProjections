...
def create_last_x(data, player, x, y=0):
    ...
    averages = player_last_x.groupby('PLAYER_NAME').mean(numeric_only=True)
    averages['Home']=player_last_x['Home'].sum()/x
...
def create_season_avg(data, player, y=0):
    ...
    averages = player_data.groupby('PLAYER_NAME').mean(numeric_only=True)
    return (averages)


def generate_x_y(data, lastx=5):
    # a task to generate data points from player lst x and plyaer seasonal averages at the time of the game
    ...
    for index, row in data.iterrows():  # go through each row, identify number of games played
        ...
        tempx = (create_last_x(data, name, lastx, (playergames +1 - playernamedict[name])))
...
def buildplayer(data,Name,Home,matchup,x): #TODO INTROUCE HOME/away
    ...
    X = pd.concat([lastx, season, pd.Series(1),dummies], axis=1) #1 temphome placeholer
